...
{
  MessagePortCommandConnect(endPoint, 
                            *this, 
                            completionHandler)();
}
    ...
    AsioExpress::CompletionHandler completionHandler)
{
  ...
  MessagePortCommandReceive(m_ioService,
                            m_receiveThread,
                            m_recvMessageQueue,
                            buffer,
                            completionHandler,
...
AsioExpress::Error MessagePort::SetupWithMessageQueues(const std::string& sendQueue, const std::string& recvQueue)
{
  ...
  try 
  {
    ...
    m_receiveThread.reset(new ReceiveThread(m_ioService, m_recvMessageQueue));
    m_sendThread.reset(new SendThread(m_ioService, m_sendMessageQueue));
  }
