...
namespace AsioExpress {
    ...
    class BasicRepeatingTimer : public boost::asio::basic_io_object<TimerService>
    {
    public:
        typedef boost::asio::basic_deadline_timer<Time, TimeTraits, TimerService>  timer_type;

        explicit BasicRepeatingTimer( boost::asio::io_service& io_service )
        ...
        void ChangeInterval( typename timer_type::duration_type const & repeat_interval )
        {
            boost::recursive_mutex::scoped_lock guard(lock_);
            if( timer_ )
            {
    ...
    typedef BasicRepeatingTimer<boost::posix_time::ptime> RepeatingTimer;
}
