...
inline UniqueEventHub<E,K>::UniqueEventHub() :
...
template<typename E, typename K>
inline typename UniqueEventHub<E,K>::ListenerPointer UniqueEventHub<E,K>::NewListener(
      Key key,
      EventPointer const & eventValue)
{
  CHECK_MSG(m_eventListeners->count(key) == 0, "A UniqueEventHub listener with this key already exists.");
...
template<typename E, typename K>
inline void UniqueEventHub<E,K>::AsyncWait(
      ListenerPointer const & listener,
      TimerPointer const & waitTimer, 
      CompletionHandler completionHandler)
{
...
template<typename E, typename K>
inline void UniqueEventHub<E,K>::Add(
      Key key,
      Event eventValue)
{
...
template<typename E, typename K>
inline void UniqueEventHub<E,K>::ShutDown()
{
